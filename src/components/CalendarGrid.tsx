    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
      ...
      <div className="grid grid-cols-7 gap-px bg-gray-200 dark:bg-gray-600">
        {['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'].map(day => (
          <div key={day} className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-700 dark:to-gray-600 px-4 py-4 text-center">
            <span className="text-sm font-bold text-gray-700 dark:text-gray-300 hidden sm:inline">{day}</span>
            <span className="text-sm font-bold text-gray-700 dark:text-gray-300 sm:hidden">{day.slice(0, 3)}</span>
          </div>
        ))}
      </div>
              ...
              {dayAppointments.length === 0 && isCurrentMonth && (
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  <div className="text-center">
                    ...
                    <span className="text-xs text-gray-400 dark:text-gray-500">Add appointment</span>
                  </div>
                </div>
              )}
            </div>
