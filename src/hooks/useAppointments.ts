...
export function useAppointments() {
  ...
  const getAppointmentsForDate = useCallback((date: string) => {
    return appointments.filter(apt => {
      ...
      const matchesDoctor = !filters.doctor || apt.doctorId === filters.doctor;
      const matchesPatient = !filters.patient || apt.patientId === filters.patient;
      return matchesDate && matchesDoctor && matchesPatient;
    });
  }, [appointments, filters]);

  const getAppointmentsForMonth = useCallback((year: number, month: number) => {
    return appointments.filter(apt => {
      const aptDate = new Date(apt.date);
      const matchesMonth = aptDate.getFullYear() === year && aptDate.getMonth() === month;
      const matchesDoctor = !filters.doctor || apt.doctorId === filters.doctor;
      const matchesPatient = !filters.patient || apt.patientId === filters.patient;
